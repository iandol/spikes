...
% Updated to support scProgressBar in sigTOOL
%
...
% If present, use the sigTOOL progress bar
try
    % sigTOOL
    if ischar(whichbar)
        fout=scProgressBar(x, whichbar,...
            'Name', 'SON Library',...
            'Position', [0.4 0.375 0.35 0.175]);
    else
        fout=scProgressBar(x, whichbar);
    end
    return
    ...
    try
        ...
        switch type
            ...
            case 2,  % waitbar(x,name)  initialize
                ...
                t=sprintf('MATLAB SON Library %3.2f © King’s College London 2002-2006',SONVersion('nodisplay'));
                text(0.2,-1,['\it' t],'FontSize',8,'Color',[0 0 0.4]);
                xpatch = [0 x x 0];
                ypatch = [0 0 1 1];
