function h = timebar(message,name)
...
%
%    The time estimate is linear using the initial time (when TIMEBAR
%    is first opened), the current time, and the percent complete.
%
...
if ~ishandle(message)                                       % If first input is not a timebar handle, 
    ...
    userdata.axes = axes('parent',h,...                     % Set the progress bar parent to the figure
        ...
        'color',[1 1 1],...                                % Set plot background color to window color
