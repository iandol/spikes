...
double variance_boot(struct hist1d *in,
		     ...
		     struct options_entropy *opts)
{
  ...
  if((*in).P==1)
      ...
      for(b=0;b<(*opts).boot_num_samples;b++)
	{
	  ...
	  for(p=0;p<(*boot).P;p++)
	    {
	      ...
	      /* The first wordcnt[0] samples refer to wordlist[0],
		 the next wordcnt[1] samples refer to wordlist[1], and so on. */
	  ...
	  for(j=0;j<(*in).C;j++)
	    {
	      /* Here's where we weed out the empty bins
