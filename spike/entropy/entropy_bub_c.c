...
void bag1(int N,double m,double *a,double lambda_0,struct bub_options *bubopts);
void bub1(int N,double m,double *a,double lambda_0,int K,int compat,struct bub_options *bubopts);
...
double entropy_bub(struct hist1d *in,struct options_entropy *opts)
{
  double *a;
  int *h;
  double H;
  ...
  if((*opts).bub_possible_words_strategy==0)
    ...
    m = MIN(max_possible_words(in),BUB_MAXWORDS_LIMIT);
  else
    m = (double)(*in).C; 
  ...
  if(N<(*bubopts).bag_threshold)
    {
      ...
      bag1(N,m,a,(*opts).bub_lambda_0,bubopts);
    }
  else
    {
      ...
      bub1(N,m,a,(*opts).bub_lambda_0,(*opts).bub_K,(*opts).bub_compat,bubopts);
    }
  ...
  for(j=0;j<=N;j++)
    {
      /* printf("a[%d]=%f h[%d]=%d a[%d]*h[%d]=%f\n",j,a[j],j,h[j],j,j,a[j]*h[j]); */
      H += a[j]*h[j];
    }  
  H = NAT2BIT(H);
