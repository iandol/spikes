classdef runExperiment < dynamicprops
	...
	%  ss.run;
	%	
	%	will run a minimal experiment showing a 1c/d grating, repeating the
	%	display
	...
	methods ( Access = protected ) %----------PRIVATE METHODS-------------------%
		...
		function initialiseTask(obj)
			...
			if obj.task.nVars == 0 %no variables
				obj.task.thisTrial=0;
			else
		...
		function updateVars(obj)
			...
			for i=1:obj.task.nVars
				...
				if (strcmp(name,'angle')||strcmp(name,'moveAngle'))
					for j=1:length(ix)
		...
		function setupBar(obj,i)
			...
			[dx dy]=pol2cart(obj.d2r(obj.sVals(i).angle+180),obj.sVals(i).startPosition);
			
			obj.sVals(i).dstRect=Screen('Rect',obj.sVals(i).texture);
			obj.sVals(i).dstRect=CenterRectOnPoint(obj.sVals(i).dstRect,obj.xCenter,obj.yCenter);
			obj.sVals(i).dstRect=OffsetRect(obj.sVals(i).dstRect,(obj.sVals(i).xPosition+dx)*obj.ppd,(obj.sVals(i).yPosition+dy)*obj.ppd);
			obj.sVals(i).mvRect=obj.sVals(i).dstRect;
		end
		...
		function drawGrid(obj)
			...
			x=x.*obj.ppd;
			Screen('DrawDots',obj.win,x,1,[0.8 0.8 0],[obj.xCenter obj.yCenter]);
		end
