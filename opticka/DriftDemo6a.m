...
texsize=400; % Half-Size of the grating image.
texsize2=100;
gap=20;
contrast = 0.36 ;
...
try
    ...
    % of the 1st texture:
    visible2size = 2*texsize2+1;
    x = meshgrid(-texsize2:texsize2 + p, -texsize2:texsize2);
    grating = gray + inc*cos(fr*x);
    ...
    % Create circular aperture for the alpha-channel:
    [x,y]=meshgrid(-texsize2:texsize2, -texsize2:texsize2);
    circle = white * (x.^2 + y.^2 <= (texsize2)^2);

    % Set 2nd channel (the alpha channel) of 'grating' to the aperture
    % defined in 'circle':
    grating(:,:,2) = 0;
    grating(1:2*texsize2+1, 1:2*texsize2+1, 2) = circle;

    % Store alpha-masked grating in texture and attach the special 'glsl'
    % texture shader to it:
    gratingtex2 = Screen('MakeTexture', w, grating, [], [], [], [], glsl);
	 
	 texsize2=texsize2+gap;
	 visible3size = 2*texsize2+1;
    x = meshgrid(-texsize2:texsize2 , -texsize2:texsize2);
	 grating = x;
    grating(:) =  gray;
    [x,y]=meshgrid(-texsize2:texsize2, -texsize2:texsize2);
    circle = white * (x.^2 + y.^2 <= (texsize2)^2);
    grating(:,:,2) = 0;
    grating(1:2*texsize2+1, 1:2*texsize2+1, 2) = circle;
    gratingtex3 = Screen('MakeTexture', w, grating, [], [], [], [], glsl);
    ...
    src2Rect=[0 0 visible2size visible2size]
	 ...
	 % Definition of the drawn source rectangle on the screen:
    src3Rect=[0 0 visible3size visible3size]
    ...
    while (vbl < vblendtime) & ~KbCheck %#ok<AND2>
        ...
        Screen('DrawTexture', w, gratingtex1, srcRect, [], angle, [], [], [], [], [], [0, yoffset, 0, 0]);
		  ...
		  % Draw first grating texture, rotated by "angle":
        Screen('DrawTexture', w, gratingtex3, src3Rect, [], 0, [], [], [], [], [], [0, 0 , 0, 0]);
